...
angular.module("schemaForm").run(["$templateCache", function ($templateCache) {
        ...
        $templateCache.put("decorators/bootstrap/table.html", "<div class='col-lg-12 col-md-12'>" +              
            "<div sf-field-model=\"\" schema-form-array  sf-field-value=\"replaceAll\" sf-array=\"form\" ng-model=\"$$value$$\">{{$$value$$}}" +
            "<div class='panel panel-info'><div class='panel-heading'>  <a class=\"btn btn-default btn-xs\" ng-click=\"collapsed = !collapsed\">" + 
            "<i ng-class=\"{'fa fa-minus': (!collapsed), 'fa fa-plus': (collapsed)}\"></i></a> {{ form.title }}</div><div class='panel-body'>" + 
            "<table ng-model=\"modelArray\"   table-directive  class=\"table table-hover table-striped table-condenes\"><thead><tr><th ng-repeat='column in columns'>{{column.headerName}}</th><th></th></tr></thead>" + 
            "<tbody ng-hide='collapsed'><tr ng-repeat='row in gridOptions.rowData' ng-dblclick=\"edit(row,form)\"><td ng-repeat='column in columns'>" + 
            "<span ng-if='column.getter && column.type !== \"string\"'>{{deepFind(row, column) }}{{ row[column.field + '_lookup'] }}</span>" + 
            "<span ng-if=\"column.filter==='date'\">{{row[column.field] | date}}</span>" + 
            "<span ng-if=\"((!column.getter ) || (column.getter && colum.type!=='string' )) && column.filter!=='date'\">{{row[column.field]}}</span></td>" +
            "<td style='white-space:nowrap;'>    " +
            "&nbsp;<a style='margin-right: 8px;' class=\"btn btn-danger btn-xs \" ng-click=\"delete(row,form,modelArray,$event)\"><i class=\"glyphicon glyphicon-trash\"></i> {{ \"Delete\" | translate }}</a>" +
            "&nbsp;<a class=\"btn btn-info btn-xs \" ng-click='edit(row,form)'><i class=\"glyphicon glyphicon-edit \"></i> {{\" Edit \" | translate}}</a> " +
            
            "</td></tr></tbody></table></div><div class='panel-footer'> " + 
            "<a ng-hide=\"form.readonly || form.add === null\" ng-click=\"add(form)\" ng-disabled=\"form.schema.maxItems <= modelArray.length\" type=\"button\" class=\" btn btn-sm btn-info  pull-right\"><i class=\"glyphicon glyphicon-plus\"></i> {{ form.add || \'Add\'}}</a>" + 
//            "<a class='btn btn-info' ng-click='add(form)'>Add</a></div></div>" + 
            "<div class=\"clearfix\"></div></div><div class=\"clearfix\" ng-model=\"modelArray\"  ><div class=\"help-block\" ng-show=\"(hasError() && errorMessage(schemaError())) || form.description\" ng-bind-html=\"(hasError() && errorMessage(schemaError())) || form.description\"></div></div></div>");
        ...
        $templateCache.put("decorators/bootstrap/select.html", 
            "<div class=\"form-group {{form.htmlClass}} schema-form-select\" ng-class=\"{\'has-error\': form.disableErrorState !== true && hasError(), \'has-success\': form.disableSuccessState !== true && hasSuccess(), \'has-feedback\': form.feedback !== false}\"><label class=\"control-label {{form.labelHtmlClass}}\" ng-show=\"showTitle()\">{{form.title}}</label><select sf-field-model=\"\" ng-disabled=\"form.readonly\" sf-changed=\"form\"" + 
			"class=\"form-control {{form.fieldHtmlClass}}\" schema-validate=\"form\" ng-options=\"item.value as item.name   for item in form.titleMap track by item.value._id\" name=\"{{form.key.slice(-1)[0]}}\"></select><div class=\"help-block\" sf-message=\"form.description\"></div></div>");
...
angular.module("schemaForm").run(["$templateCache", function (e) { e.put("directives/decorators/bootstrap/ckeditor/ckeditor.html", '<div class="form-group" ng-class="{\'has-error\': hasError()}"><label class="control-label" ng-show="showTitle()">{{form.title}}</label> <textarea ng-show="form.key" style="background-color: white" type="text" class="form-control" schema-validate="form" ng-model="$$value$$" ckeditor="form.ckeditor"></textarea> <span class="help-block">{{ (hasError() && errorMessage(schemaError())) || form.description}}</span></div>') }]), angular.module("schemaForm").config(["schemaFormProvider", "schemaFormDecoratorsProvider", "sfPathProvider", function (e, t, r) {
        ...
        }; e.defaults.string.unshift(o), t.addMapping("bootstrapDecorator", "ckeditor", "directives/decorators/bootstrap/ckeditor/ckeditor.html"), 
    t.createDirective("ckeditor", "directives/decorators/bootstrap/ckeditor/ckeditor.html")
    }]), function (e, t) {
