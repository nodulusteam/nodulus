DynamicData
.controller('EditItemCtrl', function ($scope, $Cache , $resource, $Broker, $timeout, $uibModal) {
    ...
    var colName = $Cache["schemas"][keysArr[0]].name;
    ...
    $Broker.ready(colName , keysArr[1] , function (data) {
        ...
        //for (var i = 0; i < $scope.nameFields.length; i++) {
        //    $scope.Name += $scope.Item[$scope.nameFields[i]] + " ";
    ...
    $scope.recursiveBuildForm = function (schema, form) {
    ...
    $Cache.ready("schemas", function () {
        ...
        $scope.schema = JSON.parse(JSON.stringify($Cache.schemas[keysArr[0]]));
        ...
        $scope.recursiveBuildForm($scope.schema, $scope.form);
        $timeout(function () {
            $scope.$broadcast('schemaFormValidate');
        }, 200);
    })
