    ...
    constructor() {
        ...
        for (var name of Object.keys(nodulus_modules)) {
            ...
            try {
                ...
                if (nodulus_module.routes !== undefined) {
                    for (var x = 0; x < nodulus_module.routes.length; x++) {
                        try {
                            ...
                            core.app.use('/' + npmname + nodulus_module.routes[x].route, require(pathRoute));
                        }
                        catch (error) {
                            console.error(error);
                        }
                ...
                core.app.use('/' + name, core.static(path.join(baseFolderForStatic, 'node_modules', name, 'public')));
            }
            catch (err) {
        ...
        console.log("***_____________________________________________________________________***");
        core.app.use("/nodulus", require('../routes/nodulus.js'));
        var api =  rest.start(core.app);
        console.log("***_____________________________________________________________________***");
        core.app.use('/', core.static(path.join(baseFolderForStatic, 'bower_components')));
        core.app.use('/', core.static(path.join(baseFolderForStatic, 'public')));

        core.app.use('/nodulus.json', (req: any, res: any) => {
